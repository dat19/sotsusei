...
|ステップ|キャラクターCG|プログラム|
|-:|:-|:-|
|1|・フェーズ1に必要な仮グラフィックデータをなるべく早く作成(ラフでよいので、色、大きさ、アニメの枚数を揃える)|・Unityのプロジェクトを作成して、チーム開発できるようにGitHub上で共有<br>・重要なシーンから、仮のグラフィックを表示できるような仕組みを作る|
|2|・ポートフォリオに使うことを想定して、ゲームの素材を奇麗に仕上げる<br>・組み込みチェックで挙がった修正を反映させる|・仮グラフィックを組み込み、全員で確認。修正点をリストアップ|
|3|・仮のグラフィックを仕上げる|・ゲームシステムを開発|
